...
export default class GTLFLoader {
  ...
  async load_terrain() {
    ...
    for (let i = 0; i < node?.mesh.primitives.length; i++) {
      const meshBufferView: GLTFBufferView =
        node.mesh.primitives[i].positions.bufferView;
      const componentType: GLTFComponentType =
        node.mesh.primitives[i].normals.componentType;
      const mesh: TypedArray = new (typedArrayFromComponentType(
        componentType,
      ) as any)(
        meshBufferView.view.buffer,
        meshBufferView.view.byteOffset,
        meshBufferView.view.byteLength /
          get8byteMultipleFromComponentType(componentType),
      );
  ...
  async load_terrain_normals(positionMesh: TypedArray, pIndex: number) {
    ...
    const meshNormalsBufferView: GLTFBufferView =
      node.mesh.primitives[pIndex].normals.bufferView;
    const normalComponentType: GLTFComponentType =
      node.mesh.primitives[pIndex].normals.componentType;

    const meshTangentsBufferView: GLTFBufferView =
      node.mesh.primitives[pIndex].tangents.bufferView;
    const tangentComponentType: GLTFComponentType =
      node.mesh.primitives[pIndex].tangents.componentType;

    const meshIndicesBufferView: GLTFBufferView =
      node.mesh.primitives[pIndex].indices.bufferView;
    const indexComponentType: GLTFComponentType =
      node.mesh.primitives[pIndex].indices.componentType;

    const meshUVBufferView: GLTFBufferView =
      node.mesh.primitives[pIndex].texCoords.bufferView;
    const UVComponentType: GLTFComponentType =
      node.mesh.primitives[pIndex].texCoords.componentType;

    const meshNormals: TypedArray = new (typedArrayFromComponentType(
      normalComponentType,
    ) as any)(
      meshNormalsBufferView.view.buffer,
      meshNormalsBufferView.view.byteOffset,
      meshNormalsBufferView.view.byteLength /
        get8byteMultipleFromComponentType(normalComponentType),
    );
    const meshTangents: TypedArray = new (typedArrayFromComponentType(
      tangentComponentType,
    ) as any)(
      meshTangentsBufferView.view.buffer,
      meshTangentsBufferView.view.byteOffset,
      meshTangentsBufferView.view.byteLength /
        get8byteMultipleFromComponentType(tangentComponentType),
    );
    const meshIndices: TypedArray = new (typedArrayFromComponentType(
      indexComponentType,
    ) as any)(
      meshIndicesBufferView.view.buffer,
      meshIndicesBufferView.view.byteOffset,
      meshIndicesBufferView.view.byteLength /
        get8byteMultipleFromComponentType(indexComponentType),
    );
    const meshUVs: TypedArray = new (typedArrayFromComponentType(
      UVComponentType,
    ) as any)(
      meshUVBufferView.view.buffer,
      meshUVBufferView.view.byteOffset,
      meshUVBufferView.view.byteLength /
        get8byteMultipleFromComponentType(UVComponentType),
    );
    ...
    for (let i = 0; i < meshIndices.length; i += 3) {
      ...
      const p1: Vec3 = vec3.fromValues(
        positionMesh[i1],
        positionMesh[i1 + 1],
        positionMesh[i1 + 2],
      );
      const p2: Vec3 = vec3.fromValues(
        positionMesh[i2],
        positionMesh[i2 + 1],
        positionMesh[i2 + 2],
      );
      const p3: Vec3 = vec3.fromValues(
        positionMesh[i3],
        positionMesh[i3 + 1],
        positionMesh[i3 + 2],
      );
    ...
    for (let i = 0; i < meshNormals.length; i += 3) {
      const n = vec3.normalize(
        vec3.fromValues(meshNormals[i], meshNormals[i + 1], meshNormals[i + 2]),
      );
      meshNormals[i] = n[0];
      meshNormals[i + 1] = n[1];
      meshNormals[i + 2] = n[2];
  ...
  createGridMesh(
    ...
    for (let i = 0; i < gridMesh.length; i += 3) {
      const mapSize: number =
        nodes[this.terrainNodeIndex].max[0] -
        nodes[this.terrainNodeIndex].min[0];

      const nFractAlongMeshX: number =
        (gridMesh[i] - nodes[this.terrainNodeIndex].min[0]) / mapSize;
      const nFractAlongMeshY: number =
        (gridMesh[i + 2] - nodes[this.terrainNodeIndex].min[2]) / mapSize;
  ...
  async parse_gltf(url: string): Promise<void> {
    const glb: ArrayBuffer = await fetch(`${url}.glb`).then((res) =>
      res.arrayBuffer(),
    );
    ...
    for (let m of this.jsonChunk["materials"]) {
      ...
      if ("metallicRoughnessTexture" in pbrMR) {
        metallicRoughnessTexture =
          this.textures[pbrMR["metallicRoughnessTexture"]["index"]];
      }
  ...
  load_skins() {
    for (let i = 0; i < this.jsonChunk?.["skins"]?.length; i++) {
      ...
      const inverseBindMatrices: GLTFAccessor =
        this.accessors[skin.inverseBindMatrices];
  ...
  load_animations() {
    for (let i = 0; i < this.jsonChunk?.["animations"]?.length; i++) {
      ...
      const samplers: GLTFAnimationSampler[] = this.get_animation_samplers(
        animation["samplers"],
      );
      const channels: GLTFAnimationChannel[] = this.get_animation_channels(
        animation["channels"],
        samplers,
      );

      animations[animation["name"]] = new GLTFAnimation(
        animation["name"],
        channels,
      );
    }
  ...
  load_nodes(
    ...
    const children: number[] =
      isRoot && node["children"] ? node["children"] : [];
    const name: string = node.name;
    const mesh: GLTFMesh = this.meshes[node["mesh"]] ?? null;
    const skin: GLTFSkin = this.skins[node["skin"]] ?? null;
    const minValues: Vec3[] =
      mesh?.primitives.map((p) => p.positions.min) ?? null;
    const maxValues: Vec3[] =
      mesh?.primitives.map((p) => p.positions.max) ?? null;
    ...
    else if (name === "Terrain_Materials")
      this.terrainMaterialIndex = lastNodeIndex;

    this.indexSwapBoard[n] = lastNodeIndex;

    const lightRef: number =
      node?.["extensions"]?.["KHR_lights_punctual"]?.["light"];
    if (lightRef !== undefined) {
      const lightDetails =
        this.jsonChunk["extensions"]["KHR_lights_punctual"]["lights"][lightRef];
    ...
    for (let i = 0; i < mesh?.primitives.length; i++) {
      ...
      if (prim?.material?.isTransparent) {
        this.modelNodeChunks.transparent.push({
          nodeIndex: lastNodeIndex,
          primitiveIndex: i,
        });
      } else
        this.modelNodeChunks.opaque.push({
          nodeIndex: lastNodeIndex,
          primitiveIndex: i,
        });
    }
  ...
  setup_models(): void {
    ...
    for (let i = 0; i < nodes.length; i++)
      nodes[i].terrainNodeIndex = this.terrainNodeIndex;
  }
