...
export default class GLTFTerrainMaterial {
  ...
  setMaterials(device: GPUDevice, bindGroupLayouts: BindGroupLayouts) {
    ...
    let metallicRoughnessTextureView: GPUTextureView =
      metallicRoughnessTextureArray.createView({
        format: "rgba8unorm",
        dimension: "2d-array",
        aspect: "all",
        baseMipLevel: 0,
    ...
    for (let i = 0; i < this.materials.length; i++) {
      ...
      device.queue.copyExternalImageToTexture(
        ...
        {
          texture: metallicRoughnessTextureArray,
          premultipliedAlpha: true,
          origin: { z: i },
        },
        imgSize,
    ...
    let baseColorSampler: GPUSampler =
      this.materials[0].baseColorTextureView?.sampler?.sampler;
    ...
    let normalSampler: GPUSampler =
      this.materials[0].normalTextureView?.sampler?.sampler;
