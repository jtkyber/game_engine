...
export default class GLTFNode {
  ...
  set_direction_vectors() {
    ...
    this.forwardMove = vec3.normalize(
      vec3.create(this.forward[0], 0, this.forward[2]),
    );

    this.right = vec3.normalize(vec3.cross(this.forward, [0, 1, 0]));
    this.rightMove = vec3.normalize(
      vec3.create(this.right[0], 0, this.right[2]),
    );
  ...
  move_forward(speedMult: number = 1) {
    ...
    this.position = vec3.addScaled(
      this.position,
      vec3.negate(this.forward),
      amt,
    );
  }
  ...
  rotateAroundPoint(angleRad: number, axis: Vec3, pivot: Vec3) {
    ...
    const translatedPosition: Vec3 = vec3.transformMat4(
      this.position,
      translateToOrigin,
    );
    const rotationMatrix: Mat4 = mat4.rotation(axis, angleRad);
    const rotatedPosition: Vec3 = vec3.transformMat4(
      translatedPosition,
      rotationMatrix,
    );
  ...
  set_current_velocity() {
    if (this.rootNode === null) {
      const moveDir: Vec3 = vec3.normalize(
        vec3.sub(this.position, this.previousPosition),
      );
      this._currentVelocity = vec3.scale(moveDir, this.speed);
      this._currentSpeed = Math.abs(
        vec3.dist(this.position, this.previousPosition),
      );
    }
  }
  ...
  apply_gravity() {
    if (this.rootNode === null && this.name !== "Terrain" && this.hasPhysics) {
      ...
      if (terrainHeightMap && this.terrainNodeIndex >= 0)
  ...
  limit_height_to_terrain(): void {
    ...
    const mapLength: number =
      nodes[this.terrainNodeIndex].max[0] - nodes[this.terrainNodeIndex].min[0];
    const mapWidth: number =
      nodes[this.terrainNodeIndex].max[2] - nodes[this.terrainNodeIndex].min[2];

    const nFractAlongMeshX: number =
      (this.position[0] - nodes[this.terrainNodeIndex].min[0]) / mapLength;
    const nFractAlongMeshY: number =
      (this.position[2] - nodes[this.terrainNodeIndex].min[2]) / mapWidth;

    const pLocInMapContextX: number =
      nFractAlongMeshX * (terrainHeightMapSize - 1);
    const pLocInMapContextY: number =
      nFractAlongMeshY * (terrainHeightMapSize - 1);
    ...
    for (let p of interpolationSquare) {
      const height =
        getPixel(terrainHeightMap, p[1], p[0], terrainHeightMapSize) ??
        -Infinity;
  ...
  setInitialOOB() {
    this.initialOBB.vertices[0] = vec3.create(
      this.min[0],
      this.min[1],
      this.min[2],
    );
    this.initialOBB.vertices[1] = vec3.create(
      this.max[0],
      this.min[1],
      this.min[2],
    );
    this.initialOBB.vertices[2] = vec3.create(
      this.max[0],
      this.max[1],
      this.min[2],
    );
    this.initialOBB.vertices[3] = vec3.create(
      this.min[0],
      this.max[1],
      this.min[2],
    );
    this.initialOBB.vertices[4] = vec3.create(
      this.min[0],
      this.min[1],
      this.max[2],
    );
    this.initialOBB.vertices[5] = vec3.create(
      this.max[0],
      this.min[1],
      this.max[2],
    );
    this.initialOBB.vertices[6] = vec3.create(
      this.max[0],
      this.max[1],
      this.max[2],
    );
    this.initialOBB.vertices[7] = vec3.create(
      this.min[0],
      this.max[1],
      this.max[2],
    );
  ...
  async setBoundingBoxes() {
    ...
    this.transformOBB(
      ...
      [...this.initialOBB.vertices],
      true,
    );
    this.transformOBB(
      this.normalTransform,
      [...this.initialOBB.normals],
      false,
    );
  ...
  transform_to_local_space() {
    ...
    for (let j = 0; j < this.initialOBB.vertices.length; j++) {
      this.initialOBB.vertices[j] = vec3.transformMat4(
        this.initialOBB.vertices[j],
        t,
      );
    }
