...
export default class GLTFAnimationChannel {
  ...
  play(speed: number) {
    ...
    switch (this.path) {
      case GLTFAnimationPath.TRANSLATION:
        nodes[this.targetNode].position = this.get_interpolated_value(
          3,
          times,
          values,
        );
      ...
      case GLTFAnimationPath.ROTATION:
        nodes[this.targetNode].quat = this.get_interpolated_value(
          4,
          times,
          values,
        );
      ...
      case GLTFAnimationPath.SCALE:
        nodes[this.targetNode].scale = this.get_interpolated_value(
          3,
          times,
          values,
        );
  ...
  get_value(values: any, i: number, componentCount: number) {
    return values.slice(
      i * componentCount,
      i * componentCount + componentCount,
    );
  }
  ...
  get_interpolated_value(
    ...
    const interpolation: GLTFAnimationInterpolation =
      this.sampler.interpolation;
      ...
      if (this.currentTime >= prevTime && this.currentTime <= nextTime) {
        switch (interpolation) {
          ...
          case GLTFAnimationInterpolation.LINEAR:
            const interpolationValue: number =
              (this.currentTime - prevTime) / (nextTime - prevTime);

            return this.get_lerped_value(
              prevValue,
              nextValue,
              interpolationValue,
            );
