...
export default class JointMatrices {
  ...
  constructor(device: GPUDevice) {
    ...
    this.shaderModule = <GPUShaderModule>(
      this.device.createShaderModule({ label: "shader", code: shader })
    );
  }
  ...
  setInverseBindMatricesForModel(node: GLTFNode) {
    this.inverseBindMatricesBuffer =
      node.skin.inverseBindMatrices.bufferView.gpuBuffer;
  }

  setInverseGlobalTransformForModel(
    nodeIndex: number,
    modelTransforms: Float32Array,
  ) {
    const globalTransform: Mat4 = modelTransforms.slice(
      16 * nodeIndex,
      16 * nodeIndex + 16,
    );
    ...
    this.device.queue.writeBuffer(
      this.inverseGlobalTransformBuffer,
      0,
      inverseGlobalTransform,
    );
  }

  setGlobalJointTransformsForModel(
    node: GLTFNode,
    modelTransforms: Float32Array,
  ) {
    const globalJointTransformArr: Float32Array = new Float32Array(
      16 * node.skin.joints.length,
    );
    ...
    this.device.queue.writeBuffer(
      this.globalJointTransformsBuffer,
      0,
      globalJointTransformArr,
    );
  }
  ...
  async compute(count: number) {
    ...
    const computePass = commandEncoder.beginComputePass({
      label: "jointMatricesComputePass",
    });
