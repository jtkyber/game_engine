...
export default class Renderer {
  ...
  async setupDevice() {
    ...
    this.colorVertShaderModule = <GPUShaderModule>(
      this.device.createShaderModule({
        label: "colorVertShaderModule",
        code: colorVertShader,
      })
    );
    this.colorVertShaderModuleSkinned = <GPUShaderModule>(
      this.device.createShaderModule({
        label: "colorVertShaderModuleSkinned",
        code: colorVertShaderSkinned,
      })
    );
    ...
    // );
    this.obbShaderModule = <GPUShaderModule>this.device.createShaderModule({
      label: "obbShaderModule",
      code: obbShader,
    });
    this.aabbShaderModule = <GPUShaderModule>this.device.createShaderModule({
      label: "aabbShaderModule",
      code: aabbShader,
    });
    this.cullingShaderModule = <GPUShaderModule>this.device.createShaderModule({
      label: "cullingShaderModule",
      code: cullingShader,
    });
  ...
  createPipeline(bindGroupLayouts: BindGroupLayouts) {
    ...
    const bindGroupLayoutsSkinned: GPUBindGroupLayout[] =
      bindGroupLayoutArray.concat(bindGroupLayouts.jointBindGroupLayout);
  ...
  set_joint_buffers(
    ...
    const modelIndices: IModelNodeIndices[] = modelNodeChunks.opaque.concat(
      modelNodeChunks.transparent,
    );
  ...
  renderChunk(chunkType: string, chunk: IModelNodeIndices[]) {
    for (let i = 0; i < chunk.length; i++) {
      ...
      if (
        !node.mesh ||
        this.culledModels.includes(node.rootNode ?? nodeIndex) ||
        node.hidden
      ) {
        continue;
      }
  ...
  renderBoundingBoxes(isAxisAligned: boolean) {
    for (let i = 0; i < models.length; i++) {
      ...
      if (node.hasBoundingBox) {
        this.renderPass.setVertexBuffer(
          0,
          isAxisAligned ? node.AABBBuffer : node.OBBBuffer,
        );
  ...
  shadowPass(modelNodeChunks: IModelNodeChunks, renderables: IRenderData) {
    const modelIndices = modelNodeChunks.opaque.concat(
      modelNodeChunks.transparent,
    );
  ...
  setCulledModels() {
    const boundingBoxes: Float32Array = new Float32Array(
      this.boundingBoxBuffer.size / 4,
    );

    for (let i = 0; i < models.length; i++) {
      const min: Vec3 =
        nodes[models[i]].AABB?.min ?? vec3.create(-10000, -10000, -10000);
      const max: Vec3 =
        nodes[models[i]].AABB?.max ?? vec3.create(10000, 10000, 10000);
  ...
  render = (renderables: IRenderData, modelNodeChunks: IModelNodeChunks) => {
    ...
    this.device.queue.writeBuffer(
      this.skybox.sunAboveHorizonBuffer,
      0,
      renderables.sunAboveHorizon,
    );
    this.device.queue.writeBuffer(
      this.sunAboveHorizonBuffer,
      0,
      renderables.sunAboveHorizon,
    );
    ...
    this.device.queue.writeBuffer(
      this.modelTransformsBuffer,
      0,
      renderables.nodeTransforms,
    );
    this.device.queue.writeBuffer(
      this.normalTransformBuffer,
      0,
      renderables.normalTransforms,
    );
    this.device.queue.writeBuffer(
      this.projectionViewBuffer,
      0,
      renderables.camera.view,
    );
    this.device.queue.writeBuffer(
      this.projectionViewBuffer,
      64,
      renderables.camera.projection,
    );

    this.device.queue.writeBuffer(
      this.lightTypeBuffer,
      0,
      renderables.lightTypes,
    );
    this.device.queue.writeBuffer(
      this.lightPositionBuffer,
      0,
      renderables.lightPositions,
    );
    this.device.queue.writeBuffer(
      this.lightColorBuffer,
      0,
      renderables.lightColors,
    );
    this.device.queue.writeBuffer(
      this.lightIntensityBuffer,
      0,
      renderables.lightIntensities,
    );
    this.device.queue.writeBuffer(
      this.lightDirectionBuffer,
      0,
      renderables.lightDirections,
    );
    this.device.queue.writeBuffer(
      this.lightAngleDataBuffer,
      0,
      renderables.lightAngleData,
    );
    this.device.queue.writeBuffer(
      this.lightViewProjBuffer,
      0,
      renderables.lightViewProjMatrices,
    );

    this.device.queue.writeBuffer(
      this.lightCascadeSplitsBuffer,
      0,
      renderables.camera.cascadeRadiusArr,
    );
    this.device.queue.writeBuffer(
      this.cameraPositionBuffer,
      0,
      renderables.camera.position,
    );
    ...
    this.set_joint_buffers(
      renderables.jointMatricesBufferList,
      modelNodeChunks,
    );
    ...
    if (globalToggles.antialiasing) {
      ...
      )[0].view = this.multisampleTexture.createView();
      // Set the canvas texture as the texture to "resolve" the multisample texture to.
      (
        renderPassDescriptor.colorAttachments as GPURenderPassColorAttachment[]
      )[0].resolveTarget = canvasTexture.createView();
    }

    this.renderPass = <GPURenderPassEncoder>(
      this.encoder.beginRenderPass(renderPassDescriptor)
    );
    ...
    if (globalToggles.visualizeLightFrustums) {
      ...
      loop: for (
        let i: number = 0;
        i < renderables.lightTypes.length * 6;
        i++
      ) {
        const lightIndex: number = ~~(i / 6);
        const layer: number = i % 6;
