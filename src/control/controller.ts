...
export default class Controller {
  ...
  constructor(canvas: HTMLCanvasElement, camera: Camera, player: number) {
    ...
    canvas.addEventListener("wheel", (e) => this.handleScrollWheel(e), {
      passive: true,
    });
  ...
  update() {
    ...
    const speedMult: number =
      this.controlBoard.shift && !globalToggles.forceWalking ? 3 : 1;

    const endDir: Vec3 = this.get_rotated_direction_with_forward(
      this.camera.forwardMove,
    );
    if (globalToggles.firstPersonMode)
      nodes[this.player].spin_to(this.camera.yaw);
    ...
    if (this.moveVec[0] !== 0 || this.moveVec[1] !== 0) {
      if (globalToggles.firstPersonMode) {
        ...
        if (this.moveVec[1] !== 0) {
          nodes[this.player].move(
            vec3.mulScalar(nodes[this.player].rightMove, -this.moveVec[1]),
            speedMult,
          );
        }
      } else {
        ...
        nodes[this.player].move(
          vec3.mulScalar(nodes[this.player].forwardMove, -1),
          speedMult,
        );
      }
  ...
  dir_rotated(dir: Vec3, angle: number, axis: Vec3) {
    ...
    const chunk3 = vec3.mulScalar(
      vec3.mulScalar(axis, vec3.dot(axis, dir)),
      1 - Math.cos(angle),
    );
