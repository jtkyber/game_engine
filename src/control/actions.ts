import { quat, utils, vec2, Vec2, vec3, Vec3 } from "wgpu-matrix";
...
export default class Actions {
  ...
  swim(node: GLTFNode) {
    if (this.dist2d(node.position, node.targetPosition) < 0.5) {
      node.targetPosition = this.choosePoint(
        node.maxRadius,
    ...
    const targetDirection: Vec3 = vec3.normalize(
      vec3.sub(node.targetPosition, node.position),
    );
  ...
  fly(
    ...
    if (distFromOrigin < 10)
      speedMult = Math.max(Math.sqrt(distFromOrigin / 10), 0.2);
    ...
    const determineGlideAnimation = () => {
      const delta: number =
        (performance.now() - this.birdFlapTimeStamps[node.name]) / 1000;
    ...
    const timeSinceStateChange: number =
      (performance.now() - this.birdTimeStamps[node.name] || 0) / 1000;
