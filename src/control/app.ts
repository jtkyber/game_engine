...
export default class App {
  ...
  framerateElement: HTMLElement = document.getElementById(
    "fps_counter",
  ) as HTMLElement;
  ...
  async init() {
    ...
    if (showStatus) {
      document.getElementById("status").style.display =
        showStatus === "true" ? "block" : "none";
    }

    const antialiasingCached = sessionStorage.getItem("antialiasing");
    if (antialiasingCached)
      globalToggles.antialiasing = antialiasingCached === "true";
    ...
    const gltfLoader = new GTLFLoader(
    ...
    const splatMap: GLTFImage =
      await gltfLoader.get_splat_map("dist/splat_map.png");
      ...
      switch (input.id) {
        ...
        case "fpsCap":
          input.value = Math.round(
            1000 / globalToggles.fixedTimeStep,
          ).toString();
    ...
    for (let select of selects) {
      switch (select.id) {
        ...
        case "collisionDetection":
          const select2: HTMLSelectElement = document.querySelector(
            "#collisionDetection",
          );
  ...
  loadFromSessionStorage() {
    ...
    const visualizeLightFrustumsCached = sessionStorage.getItem(
      "visualizeLightFrustums",
    );
    const lockDirectionalFrustumsCached = sessionStorage.getItem(
      "lockDirectionalFrustums",
    );
    ...
    globalToggles.fixedTimeStep =
      parseFloat(sessionStorage.getItem("fpsCap")) ||
      globalToggles.fixedTimeStep;
    this.scene.camera.setFOV(
      parseFloat(sessionStorage.getItem("fov")) ||
        utils.radToDeg(this.scene.camera.fov),
    );
    ...
    if (visualizeLightFrustumsCached) {
      globalToggles.visualizeLightFrustums =
        visualizeLightFrustumsCached === "true";
    }

    globalToggles.lockDirectionalFrustums = false;
    setTimeout(() => {
      if (lockDirectionalFrustumsCached) {
        globalToggles.lockDirectionalFrustums =
          lockDirectionalFrustumsCached === "true";
      }
    }, 1000);
    ...
    for (let m of models) {
      ...
      if (node.name === "Sun") {
        ...
        const worldDirection: Vec3 = vec3.transformMat3(
          [0, 1, 0],
          rotationMatrix,
        );
    ...
    this.framerateElement.style.display = globalToggles.showFPS
      ? "block"
      : "none";

    if (collisionDetection)
      globalToggles.collisionDetection = collisionDetection;
    ...
    if (playerRotation)
      nodes[this.scene.player].quat = JSON.parse(playerRotation);
  ...
  async fetch_and_update_parameters() {
    ...
    const modelIndices = this.scene.modelNodeChunks.opaque.concat(
      this.scene.modelNodeChunks.transparent,
    );
    ...
    camera.projection = mat4.perspectiveReverseZ(
      camera.fov,
      aspect,
      camera.near,
      camera.far,
  ...
  periodic_save() {
    sessionStorage.setItem(
      "isFirstPerson",
      JSON.stringify(globalToggles.firstPersonMode),
    );
    sessionStorage.setItem(
      "playerPosition",
      JSON.stringify(Array.from(nodes[this.scene.player].position)),
    );
    sessionStorage.setItem(
      "playerRotation",
      JSON.stringify(Array.from(nodes[this.scene.player].quat)),
    );
    sessionStorage.setItem("cameraYaw", JSON.stringify(this.scene.camera.yaw));
    sessionStorage.setItem(
      "cameraPitch",
      JSON.stringify(this.scene.camera.pitch),
    );
  }
  ...
  frame = (now: number) => {
    ...
    if (updated) {
      this.renderer.render(
        this.scene.get_render_data(),
        this.scene.modelNodeChunks,
      );
    }
    ...
    if (globalToggles.showFPS) {
      ...
      this.framerateChunk.push(
        renderDelta > globalToggles.fixedTimeStep
          ? renderDelta
          : globalToggles.fixedTimeStep,
      );
