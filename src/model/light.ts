...
export default class Light {
  ...
  constructor(
    ...
    this.angleScale =
      1 / Math.max(0.001, Math.cos(innerConeAngle) - Math.cos(outerConeAngle));
  ...
  set_lvp_matrix() {
    switch (this.type) {
      case LightType.SPOT:
        const proj: Mat4 = mat4.perspectiveReverseZ(
        ...
        if (globalToggles.visualizeLightFrustums) {
          this.inverseLightViewProjMatrices.set(
            mat4.inverse(lightViewProjMatrix),
            0,
          );
        }
        break;
      ...
      case LightType.DIRECTIONAL:
        ...
        for (let i = 0; i < this.camera.cascadeCount; i++) {
          const corners: Vec3[] = this.get_frustum_corners_world_space_temp(
            splits[i],
            splits[i + 1],
          );
          let center: Vec3 = this.get_center(corners);
          ...
          const texelsPerUnit: number = 1024 / (radius * 2);
          const scaler: Mat4 = mat4.scaling([
            texelsPerUnit,
            texelsPerUnit,
            texelsPerUnit,
          ]);
          ...
          const eye: Vec3 = vec3.add(
            center,
            vec3.mulScalar(lightDir, radius * 2),
          );
          const lightViewMatrix: Mat4 = mat4.lookAt(eye, center, [0, 1, 0]);
          const lightProjMatrix: Mat4 = mat4.ortho(
            -radius,
          ...
          const lightViewProjMatrix: Mat4 = mat4.mul(
            lightProjMatrix,
            lightViewMatrix,
          );
    ...
    for (let x = 0; x < 2; x++) {
      for (let y = 0; y < 2; y++) {
        for (let z = 0; z < 2; z++) {
          ...
          corners.push(
            vec3.create(cornerTemp[0], cornerTemp[1], cornerTemp[2]),
          );
        }
      }
    }
