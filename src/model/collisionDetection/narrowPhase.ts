...
function sat_continous(
  ...
  sweepLoop: for (let i = sweepNum; i > 0; i--) {
    ...
    const vert1: Vec3[] = OBB1.vertices.map((v) =>
      vec3.sub(v, vec3.mulScalar(moveVec1, stepRatio)),
    );
    const vert2: Vec3[] = OBB2.vertices.map((v) =>
      vec3.sub(v, vec3.mulScalar(moveVec2, stepRatio)),
    );
...
function offset_nodes(
  ...
  let velocityFactor1 =
    Math.abs(vec3.dot(node1.currentVelocity, mtv)) / Math.abs(velocityAlongMTV);
  let velocityFactor2 =
    Math.abs(vec3.dot(node2.currentVelocity, mtv)) / Math.abs(velocityAlongMTV);
  ...
  const totalFactor =
    massFactor1 + velocityFactor1 + massFactor2 + velocityFactor2;
  ...
  if (playerNode !== null) {
    const stepMax: number =
      (playerNode.AABB.max[1] - playerNode.AABB.min[1]) / 3;
...
function get_relative_velocity_along_mtv(
  v1: Vec3,
  v2: Vec3,
  mtv: Vec3,
): number {
