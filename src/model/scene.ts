...
export default class Scene {
  ...
  constructor(
    ...
    this.inverseLightViewProjMatrices = new Float32Array(
      lights.length * 16 * 6,
    );
  ...
  update() {
    ...
    const update_nodes = () => {
      for (let i = 0; i < nodes.length; i++) {
        ...
        node.normalTransform = mat4.transpose(
          mat4.invert(node.globalTransform),
        );
        for (let j = 0; j < 16; j++) {
      ...
      for (let i = 0; i < models.length; i++)
        nodes[models[i]].setBoundingBoxes();
    ...
    for (let i = 0; i < models.length; i++)
      nodes[models[i]].set_previous_position();
  ...
  set_light_data(i: number) {
    ...
    if (lightNode.name === "Flashlight") {
      this.lightIntensities[i] = globalToggles.flashlightOn
        ? light.intensity
        : 0;
    }
    ...
    if (setLVP) {
      ...
      if (globalToggles.visualizeLightFrustums) {
        this.inverseLightViewProjMatrices.set(
          light.inverseLightViewProjMatrices,
          i * 16 * 6,
        );
      }
    }
    ...
    if (lightNode.name === "SunLight") {
      const sunAboveHorizon: number = vec3.dot(
        vec3.normalize(light.forward),
        [0, 1, 0],
      );
      ...
      let newColor: Vec3 = vec3.lerp(
        light.color,
        vec3.create(1.0, 0.4, 0.1),
        lerpValue,
      );
  ...
  get_node_transform(
    ...
    if (!this.allJoints.has(originalNodeIndex) && isJoint && !parentIsJoint) {
      ...
      const combinedTransform: Mat4 = mat4.mul(
        nodes[parent].transform,
        transform,
      );
      return this.get_node_transform(
        parent,
        combinedTransform,
        originalNodeIndex,
      );
  ...
  sortTransparent() {
    this.modelNodeChunks.transparent = this.modelNodeChunks.transparent.sort(
      (a, b) => {
        const nodeAdist: number = vec3.dist(
          this.camera.position,
          nodes[a.nodeIndex].position,
        );
        const nodeBdist: number = vec3.dist(
          this.camera.position,
          nodes[b.nodeIndex].position,
        );
