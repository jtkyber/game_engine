...
export class Camera {
  ...
  setFOV(angleDeg: number) {
    ...
    this.projection = mat4.perspectiveReverseZ(
      this.fov,
      aspect,
      this.near,
      this.far,
  ...
  update(terrainNodeIndex: number) {
    ...
    this.pitch = Math.max(
      Math.min(this.pitch, Math.PI / 2 - 0.001),
      -Math.PI / 2 + 0.001,
    );
    ...
    if (terrainHeightMap && terrainNodeIndex >= 0)
  ...
  limit_height_to_terrain(terrainNodeIndex: number): void {
    const mapLength: number =
      nodes[terrainNodeIndex].maxValues[0][0] - nodes[terrainNodeIndex].min[0];
    const mapWidth: number =
      nodes[terrainNodeIndex].maxValues[0][2] - nodes[terrainNodeIndex].min[2];

    const nFractAlongMeshX: number =
      (this.position[0] - nodes[terrainNodeIndex].min[0]) / mapLength;
    const nFractAlongMeshY: number =
      (this.position[2] - nodes[terrainNodeIndex].min[2]) / mapWidth;
    ...
    if (this.position[1] < terrainHeight + 0.8)
      this.position[1] = terrainHeight + 0.8;
  }
